...
% \section{Key Points}
% \begin{enumerate}
%     \item Lagged correlations between PM values and other gases/pollutants.
%     \item Gridded+Non-gridded data.
%     \item Wind based modeling.
%     \item Check uncertainty of the model as a metric.
%     \item Use Dense Elevation data.
% \end{enumerate}

% \subsection{Pitch}
% \begin{enumerate}
%     \item We can use NASA GEOS-CF data because it is available globally but we are limited by 0.25 deg resolution.
%     \item We can use Sarath's meteorological data given that it is available at 0.01 degree resolution.
% \end{enumerate}

% \subsection{Ideas}
% \begin{enumerate}
%     \item Use multiple encoders for Non-colocated inputs, concatenate and pass to decoder.    
%     \item Try multi-output modeling with multiple pollutants.
%     \item We fit a neural net to a big vector of features. What if we remove all temporal and spatial features (time, lat, lon)? What if we remove only temporal or only spatial features? What if we keep both? We can show this analysis.
% \end{enumerate}

% \subsection{Key predictors}
% \begin{itemize}
%     \item PBLH from Sarath's data.
%     \item elevation.
%     \item population dataset.
%     \item Distance from major roads.
%     \item AOD from satellite.
%     \item Power plants + their production + wind direction.
%     \item Traffic data from R package.
%     \item Fire counts.
% \end{itemize}
...
% \begin{itemize}
%     \item Delhi's data has poor meteorological data. Out of 49 stations, only 33 have missing data 10\% or less. 14 stations have 100\% missing meteorological data.
%     \item We can use ERA5 available at 0.1 degree, but they are shifting the server and thus requests are not going through.
%     \item Only option we have is Sarath's WRF outputs. We can argue that anyone can use GFS and run WRF on their region to get the meteorological data that we have.
% \end{itemize}

% \subsubsection{Traffic data}
% Traffic API by Google and also by the googletraffic R package only provides real-time data.

% \subsubsection{Road Networks}
% Sarath's website has road networks data \href{https://urbanemissions.info/india-air-quality/india-ncap-airshed-osmroads/}{[click here]}.

% \subsubsection{Problems with existing methods}
% \begin{itemize}
%     \item Random Forest does not have spatial granularity and thus can predict same values for many places. In a way, it is piece-wise mean fitting to the surface.
%     \item GraphNPs blindly use meteorological variables.
%     \item Gaussian NPs from wessel's library can not be used natively with more than 3 features. It uses Conv2d for 2 features, Conv3d for 3 features and so on.
% \end{itemize}

% \subsubsection{Extra}
% \begin{itemize}
%     \item 1 nearest neighbor interpolation is also known as the Thiessen interpolation. Pronounces as "Thay (like bye) son".
% \end{itemize}

% \subsubsection{What goes into the paper?}
% \begin{itemize}
%     \item Ghazipur case study - CO
%     \item NO2 - Anand Vihar
% \end{itemize}



\section{Introduction}
Air pollution takes 7 million lives a year and 9 out of 10 people breathe air that violates WHO standards~\cite{who}.

considered, by the United Nations, one of the major environmental risk to health worldwide. Accordingly, it is addressed for mitigation in multiple United Nations Sustainable Development Goals, such as the 3.9 and 11.6~\cite{rafaj2018outlook}.


\nb{
\section{placeholder for some points}
\begin{enumerate}
    \item Physical models interpretable but often exhibit high bias ..
    \item ML models may be able to accurately spatio-temporal interpolation but offer little actionable insights/what-ifs 
    \item PM like pollutants created from primary pollutants over several days
    \item Different sources contribute different kinds of pollutants: waste burning primarily ..
    \item Atmosphere exhibits inertia (how to model?)
    \item mixing height varies over space and time
    \item proxies for different sources: fire count + intensity for fires, traffic intensity (fine-grained), road network (coarse grained plus static) for number of vehicle, 
    \item Ghazipur land fill case
    \item Anand Vihar case
    \item low pollution case
    \item what-if: if we reduce the thermal produce by half then what is the effect?
\end{enumerate}
...
\section{A Primer on Air pollution}
...
On 28$^{th}$ March, 2022, Ghazipur landfill caught an open fire which lasted over 48 hours and was reported by multiple media houses~\cite{ndtvHoursFire,indiatodayMassiveBlaze,hindustantimesHighlightsGhazipur}.\the\linewidth

\begin{figure}
    \centering
    \includegraphics{figures/ghazipur_locs.pdf}
    \includegraphics{figures/ghazipur_co.pdf}
    \caption{Caption}
    \label{fig:enter-label}
\end{figure}

\subsubsection{Proxy: Fire counts}

\zp{\url{https://www.hindustantimes.com/cities/delhi-news/ghazipur-landfill-burns-for-3rd-time-in-a-month-101650492209448.html} 3 times burning during Apr. Another one \url{https://www.thehindu.com/news/cities/Delhi/fire-breaks-out-at-ghazipur-landfill-site-mcd-says-60-70-doused/article66962224.ece}}
